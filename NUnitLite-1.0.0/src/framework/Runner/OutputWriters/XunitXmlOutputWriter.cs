...
namespace NUnitLite.Runner
{
    ...
    public class XunitXmlOutputWriter : OutputWriter
    {
        ...
        {
            if (result.Test is TestSuite && (result.Test as TestSuite).TestType == "Assembly") {
            ...
            foreach (string key in properties.Keys)
            {
                if (key != PropertyNames.Category)
                {
                    ...
                    foreach (object prop in properties[key])
                    {
                        ...
                        xmlWriter.WriteAttributeString("name", (key == PropertyNames.Category ? "category:" : "property:") + key);
